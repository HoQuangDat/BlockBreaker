...
// var start;
let splicedSound = new Audio('sounds/splatter.mp3');
...
function renderScoreHud() {
	if (isCasualGame()) {
		scoreNode.innerText = `SCORE: ${state.game.cubeCount*3}`;
	...
	cubeCountNode.innerText = `SCORE: ${state.game.cubeCount*3}`;
...
handleClick($('.play-normal-btn'), () => {
    ...
    // Thiết lập các trạng thái của game
...
function endGame() {
    ...
    // // Dừng phát nhạc khi game over
    // if (startSound) {
    //     startSound.pause();
    //     startSound.currentTime = 0; // Đặt lại thời gian phát về đầu
    // }
    // Phát âm thanh game over
    // overSound = new Audio('sounds/over.mp3');
    // overSound.play();

	 if (window.parent) {
        window.parent.postMessage({ type: 'gameScore', score: state.game.score }, '*');
    }
}
...
// Tự động bắt đầu trò chơi khi trang được tải
